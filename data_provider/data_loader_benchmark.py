...
class CIDatasetBenchmark(Dataset):
    ...
    def __read_data__(self):
        ...
        #添加原始时间
         # 保留原始时间戳
        self.original_timestamps = df_raw['date'].values  # 假设时间戳列名为 'date'
        ...
        #修改
        self.original_timestamps=self.original_timestamps[border1:border2]
    ...
    def __getitem__(self, index):
        ...
        # # 原始时间戳
        #  original_x_timestamps = self.original_timestamps[s_begin:s_end]
        #  original_y_timestamps = self.original_timestamps[r_begin:r_end]
...
class CIAutoRegressionDatasetBenchmark(CIDatasetBenchmark):
    def __init__(self, root_path='dataset', flag='train', input_len=None, label_len=None, pred_len=None, rev_flag=False,
                 data_type='custom', scale=True, timeenc=1, freq='h', stride=1, subset_rand_ratio=1.0):
        ...
        if self.rev_flag == True:
            self.n_timepoint = self.n_timepoint - self.pred_len
    ...
    def __getitem__(self, index):
        ...
        if self.rev_flag == False:
            ...
            s_begin = index % self.n_timepoint   # select start time
            s_end = s_begin + self.input_len #
            r_begin = s_end - self.label_len #seq 192 label 96
        ...
        elif self.rev_flag == True :
            c_begin = index // self.n_timepoint  # select variable
            p_begin = index % self.n_timepoint   # select start time
            p_end = p_begin + self.input_len
            s_begin = p_end - self.label_len
            s_end = s_begin + self.label_len + self.pred_len
            ...
            seq_y_rev = self.data_x[p_begin:p_end, c_begin:c_begin + 1][::-1].copy()
            seq_x_rev = self.data_y[r_begin:r_end, c_begin:c_begin + 1][::-1].copy()
            seq_y_mark_rev = self.data_stamp[p_begin:p_end][::-1].copy()
            seq_x_mark_rev = self.data_stamp[r_begin:r_end][::-1].copy()

            return seq_x, seq_y, seq_x_mark, seq_y_mark, seq_x_rev, seq_y_rev, seq_x_mark_rev, seq_y_mark_rev
        
class CIAutoRegressionDatasetTuner(CIDatasetBenchmark):
    def __init__(self, start_idx, end_idx, root_path='dataset', flag='train', input_len=None, label_len=None, pred_len=None,
                 data_type='custom', scale=True, timeenc=1, freq='h', stride=1, subset_rand_ratio=1.0):
        self.label_len = label_len
        self.start_idx = start_idx
        self.end_idx = end_idx
        self.valid_length = end_idx - start_idx + 1
        super().__init__(root_path=root_path, flag=flag, input_len=input_len, pred_len=pred_len,
                         data_type=data_type, scale=scale, timeenc=timeenc, freq=freq, stride=stride,
                         subset_rand_ratio=subset_rand_ratio)
    ...
    def __getitem__(self, index):
        ...
        assert self.start_idx <= index <= self.end_idx, "Index out of range for the specified segment."
