    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for output_ptr in range(len(self.args.output_len_list)):
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, \
                    ...
                    batch_x = batch_x.float().to(self.device)
                    batch_y = batch_y.float().to(self.device)
                    batch_x_rev = batch_x_rev.float().to(self.device)
                    batch_y_rev = batch_y_rev.float().to(self.device)
                    
                    print(i)
                    print(f"revy:{batch_x_rev.shape}")
                    ...
                    dec_inp_rev = torch.cat([batch_y_rev[:, :self.args.label_len, :], dec_inp_rev], dim=1).float().to(self.device)
                    ...
                    for j in range(inference_steps):
                        if len(pred_y) != 0:
                            ...
                            batch_x_rev = torch.cat([batch_x_rev[:, self.args.pred_len:, :], pred_y_rev[-1]], dim=1)
                            tmp = batch_y_mark[:, j - 1:j, :]
                            tmp_rev = batch_y_mark_rev[:, j - 1:j, :]
                            batch_x_mark = torch.cat([batch_x_mark[:, 1:, :], tmp], dim=1)
                            batch_x_mark_rev = torch.cat([batch_x_mark_rev[:, 1:, :], tmp_rev], dim=1)

                        if self.args.output_attention:
                            outputs, attns, dec_out_save = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                            outputs_rev, attns_rev, dec_out_save_rev = self.model(batch_x_rev, batch_x_mark_rev, dec_inp_rev, batch_y_mark_rev)
                                                       
                        else:
                            outputs, dec_out_save = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                            outputs_rev, dec_out_save_rev = self.model(batch_x_rev, batch_x_mark_rev, dec_inp_rev, batch_y_mark_rev)
                            print(f"shape:{outputs.shape, batch_y.shape}")
                            print(f"shape_rev:{outputs_rev.shape, batch_y_rev.shape}")

                        f_dim = -1 if self.args.features == 'MS' else 0
                        pred_y.append(outputs[:, -self.args.pred_len:, :])
                        pred_y_rev.append(outputs_rev[:, -self.args.pred_len:, :])
                    pred_y = torch.cat(pred_y, dim=1)
                    pred_y_rev = torch.cat(pred_y_rev, dim=1)

                    if dis != 0:
                        pred_y = pred_y[:, :-dis, :]
                        pred_y_rev = pred_y_rev[:, :-dis, :]

                    if self.args.use_ims:
                        batch_y = batch_y[:, self.args.label_len:self.args.label_len + self.args.output_len, :].to(
                            self.device)
                        batch_y_rev = batch_y_rev[:, self.args.label_len:self.args.label_len + self.args.output_len, :].to(
                            self.device)                    
                    else:
                        batch_y = batch_y[:, :self.args.output_len, :].to(self.device)
                        batch_y_rev = batch_y_rev[:, :self.args.output_len, :].to(self.device)
                    outputs = pred_y.detach().cpu()
                    outputs_rev = pred_y_rev.detach().cpu()
                    batch_y = batch_y.detach().cpu()
                    batch_y_rev = batch_y_rev.detach().cpu()

                    if test_data.scale and self.args.inverse:
                        shape = outputs.shape
                        shape_rev = outputs_rev.shape                        
                        outputs = test_data.inverse_transform(outputs.squeeze(0)).reshape(shape)
                        batch_y = test_data.inverse_transform(batch_y.squeeze(0)).reshape(shape)
                        outputs_rev = test_data.inverse_transform(outputs_rev.squeeze(0)).reshape(shape_rev)
                        batch_y_rev = test_data.inverse_transform(batch_y_rev.squeeze(0)).reshape(shape_rev)                        

                    outputs = outputs[:, :, f_dim:]
                    batch_y = batch_y[:, :, f_dim:]
                    outputs_rev = outputs_rev[:, :, f_dim:]
                    batch_y_rev = batch_y_rev[:, :, f_dim:]

                    pred = outputs
                    true = batch_y
                    pred_rev = outputs_rev
                    true_rev = batch_y_rev


                    # import csv
                    # for line_index, (aoutputs, abatch_y, aoutputs_rev, abatch_y_rev) \
                    #     in enumerate(zip(pred, true, pred_rev, true_rev)):

                    #     mse_rev = torch.mean((aoutputs_rev - abatch_y_rev) ** 2)
                    #     mae_rev = torch.mean(torch.abs(aoutputs_rev - abatch_y_rev))

                    #     mse = torch.mean((aoutputs - abatch_y) ** 2)
                    #     mae = torch.mean(torch.abs(aoutputs - abatch_y))
                        
                    #     with open(f"pred_result_for_{self.args.data}.csv", 'a', newline='') as csvfile:
                    #         csvwriter = csv.writer(csvfile)
                    #         csvwriter.writerow([(i) * self.args.batch_size + (line_index + 1), float(mse), float(mae), float(mse_rev), float(mae_rev)])
        ...
        if self.args.output_len_list is not None:
            for i in range(len(preds_list)):
                ...
                preds_rev = torch.cat(preds_rev, dim=0).numpy()
                trues_rev = torch.cat(trues_rev, dim=0).numpy()
                mae, mse, rmse, mape, mspe = metric(preds, trues)
                mae_rev, mse_rev, rmse_rev, mape_rev, mspe_rev = metric(preds_rev, trues_rev)
